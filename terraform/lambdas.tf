...
##       USER DATA          ##
##############################
resource "aws_lambda_function" "update_user_data" {

  function_name     = "${var.app_name}-update-user-data"
  filename          = "./templates/lambda_stub.zip"
  source_code_hash  = filebase64sha256("./templates/lambda_stub.zip")
  handler           = "handler.handler"
  layers            = [aws_lambda_layer_version.lambda_layer.arn]
  ...
  tags = merge(local.standard_tags, tomap({"name" = "${var.app_name}-update-user-data"}))
...
resource "aws_lambda_function" "get_user_data" {
  ...
  function_name     = "${var.app_name}-get-user-data"
  filename          = "./templates/lambda_stub.zip"
  source_code_hash  = filebase64sha256("./templates/lambda_stub.zip")
  handler           = "handler.handler"
  layers            = [aws_lambda_layer_version.lambda_layer.arn]
  ...
  tags = merge(local.standard_tags, tomap({"name" = "${var.app_name}-get-user-data"}))
